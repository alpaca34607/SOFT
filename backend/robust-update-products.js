...
async function updateProducts() {
    try {
        ...
        console.log('📋 資料庫表格:', tables.map(t => t.name));
        ...
        for (const product of products) {
            ...
            const existing = await new Promise((resolve, reject) => {
                db.get('SELECT id FROM products WHERE product_id = ?', [product.product_id], (err, row) => {
                    if (err) reject(err);
                    else resolve(row);
                });
            });
        ...
        const allProducts = await new Promise((resolve, reject) => {
            db.all('SELECT product_id, name, thumbnail_path, lightslider_images, sketchfab_background FROM products', (err, rows) => {
                if (err) reject(err);
                else resolve(rows);
            });
        });
        ...
        allProducts.forEach(product => {
            ...
            console.log(`    LightSlider: ${product.lightslider_images ? '有' : 'null'}`);
            console.log(`    Sketchfab背景: ${product.sketchfab_background || 'null'}`);
        });
